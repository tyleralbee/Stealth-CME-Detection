...
__author__ = 'Tyler J Albee'
__contact__ = 'tyal7988@colorado.edu'


def godsPlan(start_timestamp='2010-08-07 17:12:11', end_timestamp='2010-08-07 21:18:11', output_path='/Users/tyleralbee/Desktop/StealthCME', verbose=True):
    ...
    if verbose:
        logger = JpmLogger(filename='do_stealth_cme_search', path=output_path, console=True)
    ...
    for i in range(1, numSlices):
        ...
        # ---------Fit light curves to reduce noise---------------------------------------------------------------------
            ...
            for i, column in enumerate(event_time_slice_percentages):
                if event_time_slice_percentages[column].isnull().all().all():
                    if verbose:
                        logger.info(
                            'Event {0} {1} fitting skipped because all irradiances are NaN.'.format(i, column))
                else:
                    ...
                    event_time_slice_fitted = event_time_slice_percentages

                    if verbose:
                        logger.info('Event {0} {1} light curves fitted.'.format(i, column))
...
if __name__ == "__main__":
    print("theTruth.py is being run directly")
    godsPlan(verbose=True, start_timestamp='2010-08-07 17:12:11', end_timestamp='2010-08-07 23:18:11')

else:
    print("theTruth.py is being imported into another module")
